<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-2 font-display text-3xl font-black text-casa-ink">{{ $service->name }}</h1>
    ...
    <div class="space-y-6">
        ...
        <section class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <x-app-card>
                <div class="flex flex-col gap-3 border-b border-casa-border pb-5 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <h2 class="mt-2 font-display text-xl font-black text-casa-ink">{{ __('Service information') }}</h2>
                    </div>
                ...
                <dl class="mt-5 grid gap-4 sm:grid-cols-2">
                    <div class="rounded-2xl bg-casa-bg p-4">
                        <dt class="text-sm font-black uppercase tracking-[0.12em] text-casa-muted">{{ __('Slug') }}</dt>
                        <dd class="mt-2 font-semibold text-casa-ink">{{ $service->slug }}</dd>
                    </div>
                    <div class="rounded-2xl bg-casa-bg p-4">
                        <dt class="text-sm font-black uppercase tracking-[0.12em] text-casa-muted">{{ __('Appointments') }}</dt>
                        <dd class="mt-2 font-semibold text-casa-ink">{{ trans_choice(':count record|:count records', $service->appointments_count) }}</dd>
                    </div>
                    <div class="rounded-2xl bg-casa-bg p-4 sm:col-span-2">
                        <dt class="text-sm font-black uppercase tracking-[0.12em] text-casa-muted">{{ __('Description') }}</dt>
            ...
            <aside class="casa-dark-panel rounded-[24px] p-6 shadow-casa-card">
                <p class="text-sm font-black uppercase tracking-[0.18em] text-casa-brass">{{ __('Booking readiness') }}</p>
                <h2 class="mt-4 font-display text-2xl font-black text-white">{{ __('Connected to scheduling.') }}</h2>
                <p class="mt-4 text-sm leading-7 text-casa-bg/80">
                    {{ __('Bookable therapists who offer this service can receive immediately confirmed appointments during their available hours.') }}
                </p>
                <div class="casa-divider my-6"></div>
                <div class="space-y-3 text-sm font-semibold text-casa-bg/80">
                    <p>{{ __('Therapist eligibility') }}</p>
                    <p>{{ __('Weekly availability') }}</p>
                    <p>{{ __('Confirmed appointments') }}</p>
                </div>
            </aside>
        </section>
    </div>
